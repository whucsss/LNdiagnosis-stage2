...
def identity_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = Conv2D(nb_filter1, (1, 1), name=conv_name_base + '2a',
                  use_bias=use_bias, kernel_regularizer=regularizers.l2(0.001))(input_tensor)
    x = BatchNormalization(name=bn_name_base + '2a')(x, training=train_bn)
    x = Activation('relu')(x)

    x = Conv2D(nb_filter2, (kernel_size, kernel_size), padding='same',
                  name=conv_name_base + '2b', use_bias=use_bias, kernel_regularizer=regularizers.l2(0.001))(x)
    x = BatchNormalization(name=bn_name_base + '2b')(x, training=train_bn)
    x = Activation('relu')(x)

    x = Conv2D(nb_filter3, (1, 1), name=conv_name_base + '2c',
                  use_bias=use_bias, kernel_regularizer=regularizers.l2(0.001))(x)
...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = Conv2D(nb_filter1, (1, 1), strides=strides,
                  name=conv_name_base + '2a', use_bias=use_bias, kernel_regularizer=regularizers.l2(0.001))(input_tensor)
    x = BatchNormalization(name=bn_name_base + '2a')(x, training=train_bn)
    x = Activation('relu')(x)
    ...
    x = Conv2D(nb_filter3, (1, 1), name=conv_name_base +
                  '2c', use_bias=use_bias, kernel_regularizer=regularizers.l2(0.001))(x)
    x = BatchNormalization(name=bn_name_base + '2c')(x, training=train_bn)

    shortcut = Conv2D(nb_filter3, (1, 1), strides=strides,
                         name=conv_name_base + '1', use_bias=use_bias, kernel_regularizer=regularizers.l2(0.001))(input_tensor)
...
def get_resnet(input_image,stage5=False, train_bn=True):
    ...
    x = Conv2D(64, (7, 7), strides=(2, 2), name='conv1', use_bias=True, kernel_regularizer=regularizers.l2(0.001))(x)
