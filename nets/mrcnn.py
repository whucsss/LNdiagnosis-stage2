...
def rpn_graph(feature_map, anchors_per_location):
    ...
    shared = Conv2D(512, (3, 3), padding='same', activation='relu',
                       name='rpn_conv_shared', kernel_regularizer=regularizers.l2(0.001))(feature_map)
    
    x = Conv2D(2 * anchors_per_location, (1, 1), padding='valid',
                  activation='linear', name='rpn_class_raw', kernel_regularizer=regularizers.l2(0.001))(shared)
    ...
    x = Conv2D(anchors_per_location * 4, (1, 1), padding="valid",
                  activation='linear', name='rpn_bbox_pred',  kernel_regularizer=regularizers.l2(0.001))(shared)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    x = TimeDistributed(Conv2D(fc_layers_size, (pool_size, pool_size), padding="valid",  kernel_regularizer=regularizers.l2(0.001)),
    ...
    mrcnn_class_logits = TimeDistributed(Dense(num_classes, kernel_regularizer=regularizers.l2(0.001)),
    ...
    x = TimeDistributed(Dense(num_classes * 4, activation='linear', kernel_regularizer=regularizers.l2(0.001)),
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    # Shape: [batch, num_rois, MASK_POOL_SIZE, MASK_POOL_SIZE, channels]
    x = TimeDistributed(Conv2D(256, (3, 3), padding="same",  kernel_regularizer=regularizers.l2(0.001)),
    ...
    x = TimeDistributed(Conv2DTranspose(256, (2, 2), strides=2, activation="relu", kernel_regularizer=regularizers.l2(0.001)),
                           name="mrcnn_mask_deconv")(x)
    # 反卷积后再次进行一个1x1卷积调整通道，使其最终数量为numclasses，代表分的类
    x = TimeDistributed(Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid", kernel_regularizer=regularizers.l2(0.001)),
                           name="mrcnn_mask")(x)
    return x
...
def get_predict_model(config):
    ...
    P5 = Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c5p5', kernel_regularizer=regularizers.l2(0.001))(C5)
    # P4长宽共压缩了4次
    # Height/16,Width/16,256
    P4 = Add(name="fpn_p4add")([
        UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
        Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c4p4', kernel_regularizer=regularizers.l2(0.001))(C4)])
    # P4长宽共压缩了3次
    # Height/8,Width/8,256
    P3 = Add(name="fpn_p3add")([
        UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P4),
        Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c3p3', kernel_regularizer=regularizers.l2(0.001))(C3)])
    # P4长宽共压缩了2次
    # Height/4,Width/4,256
    P2 = Add(name="fpn_p2add")([
        UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
        Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c2p2', kernel_regularizer=regularizers.l2(0.001))(C2)])
        
    # 各自进行一次256通道的卷积，此时P2、P3、P4、P5通道数相同
    # Height/4,Width/4,256
    P2 = Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p2", kernel_regularizer=regularizers.l2(0.001))(P2)
    # Height/8,Width/8,256
    P3 = Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p3", kernel_regularizer=regularizers.l2(0.001))(P3)
    # Height/16,Width/16,256
    P4 = Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p4", kernel_regularizer=regularizers.l2(0.001))(P4)
    # Height/32,Width/32,256
    P5 = Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p5", kernel_regularizer=regularizers.l2(0.001))(P5)
